...
std::runtime_error ExplicitStateDfaAdd::bad_file_format_exception(
...
std::vector<std::string> ExplicitStateDfaAdd::read_mona_line(
    std::istream& in,
    std::size_t line_number) {
  std::string line;
  std::getline(in, line);
...
std::vector<std::string> ExplicitStateDfaAdd::read_variable_names(
    std::istream& in,
    std::size_t line_number) {
...
std::size_t ExplicitStateDfaAdd::read_state_count(std::istream& in,
                                                  std::size_t line_number) {
  std::vector<std::string> substr = read_mona_line(in, line_number);
...
std::vector<std::size_t> ExplicitStateDfaAdd::read_final_states(
    std::istream& in,
    std::size_t line_number) {
  std::vector<std::string> substr = read_mona_line(in, line_number);
...
std::vector<std::size_t> ExplicitStateDfaAdd::read_behaviour(
    std::istream& in,
    std::size_t line_number) {
  std::vector<std::string> substr = read_mona_line(in, line_number);
...
std::vector<std::vector<int>> ExplicitStateDfaAdd::read_node_table(
    std::istream& in,
    std::size_t line_number) {
  std::string line;
  std::getline(in, line);
...
std::string ExplicitStateDfaAdd::ltlf_to_dfa_file(const std::string& ltlf_filename) {
...
std::string ExplicitStateDfaAdd::safetyltl_to_dfa_file(const std::string& safetyltl_filename) {
...
CUDD::ADD ExplicitStateDfaAdd::build_add_from_dfa_mona(unsigned node_index, const std::shared_ptr<VarMgr> &var_mgr,
                                                       const std::vector<std::string> &variable_names,
                                                       const ExplicitStateDfa& explicit_dfa,
                                                       std::unordered_map<std::size_t, CUDD::ADD> &add_table) {
...
ExplicitStateDfaAdd ExplicitStateDfaAdd::read_from_file(
...
ExplicitStateDfaAdd ExplicitStateDfaAdd::complement_dfa(ExplicitStateDfaAdd& d) {
    ...
    ExplicitStateDfaAdd dfa(d.var_mgr());
