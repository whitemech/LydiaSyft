...
//        res1.dfa_print();
    ...
    std::vector<std::string> ExplicitStateDfaMona::traverse_bdd(CUDD::BDD curr, std::shared_ptr<VarMgr> var_mgr, std::vector<std::string>& names,  std::string guard_str) {
...
//        res1.dfa_print();
    ...
    ExplicitStateDfaMona ExplicitStateDfaMona::dfa_product_and(const std::vector<ExplicitStateDfaMona>& dfa_vector) {
    ...
    // all the names may not be the same, needs a map for right indices
    ExplicitStateDfaMona ExplicitStateDfaMona::dfa_product_or(const std::vector<ExplicitStateDfaMona>& dfa_vector) {
        // first record all variables, as they may not have the same alphabet
        std::unordered_map<std::string, int> name_to_index = {};
        std::vector<std::string> name_vector;
        std::set<std::string> names;
    ...
    ExplicitStateDfaMona ExplicitStateDfaMona::dfa_complement(ExplicitStateDfaMona &d) {
        dfaNegation(d.dfa_);
        ExplicitStateDfaMona res_dfa(d.dfa_, d.names);
        return res_dfa;
    }
