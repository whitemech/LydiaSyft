...
namespace Syft {
    ...
    Parser Parser::read_from_file(const std::string &filename) {
        Parser parser;

        std::string cmd_get_formula = "./syfco --format ltlxba-fin -m fully "+filename;
        std::string formula_str = parser.exec(cmd_get_formula.c_str());
        parser.formula = parser.trim(formula_str);

        std::string cmd_get_ins = "./syfco --format ltlxba-fin --print-input-signals "+filename;
        ...
        std::string cmd_get_outs = "./syfco --format ltlxba-fin --print-output-signals "+filename;
        ...
        std::string cmd_get_target = "./syfco  --format ltlxba-fin -g "+filename;
